...
class KafkaConsumer {
  ...
  private def readBatchFromTopic(topic: String, iterator: ConsumerIterator[Array[Byte], Array[Byte]]) = {
    ...
    if (batch.isEmpty) throw new IllegalArgumentException(s"$topic is  empty")else{ MongoOperation.insertBooks(batch)}
    println(s"consumed batch into mongodb ::::::::::: ${batch.length}         ${batch.apply(0)}")
  }
...
object MongoKafkaConsumer extends App {
